...
import com.fk.weather.databinding.ItemSearchCityRvBinding;
...
public class SearchCityAdapter extends RecyclerView.Adapter<SearchCityAdapter.ViewHolder> {
    ...
    public static CharSequence matcherSearchText(String string, String keyWord) {
        ...
        if (indexOf != -1) {
            builder.setSpan(new ForegroundColorSpan(ContextCompat.getColor(WeatherApp.getContext(),
                    R.color.yellow)), indexOf, indexOf + keyWord.length(), SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        return builder;
    }
